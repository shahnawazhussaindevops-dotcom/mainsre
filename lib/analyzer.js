...
function buildPrompt(log, server) {
  const ctx = server
    ? `Server: ${server.name || server.host} (${server.host}). ` +
      `CPU ${fmtPct(server.cpuPct)}, memory ${fmtPct(server.memPct)}, disk ${fmtPct(server.diskPct)}.`
    : 'No live metrics available.';
  return (
    `You are a senior Site Reliability Engineer. Analyze the incident below and respond ONLY with a JSON ` +
    `object, no prose, using exactly these keys: severity ("info"|"warning"|"critical"), summary (one sentence), ` +
    `rootCause (one or two sentences), command (a single shell command to investigate or safely remediate), ` +
...
async function viaAnthropic(log, server, key) {
  ...
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      model,
      max_tokens: 512,
      messages: [{ role: 'user', content: buildPrompt(log, server) }],
    }),
  });
...
async function viaOpenAI(log, server, key) {
  ...
  const res = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        { role: 'user', content: buildPrompt(log, server) },
      ],
    }),
  });
...
export async function analyze({ log, server }) {
  const anthropicKey = process.env.ANTHROPIC_API_KEY?.trim();
  const openaiKey = process.env.OPENAI_API_KEY?.trim();
  try {
    if (anthropicKey) return await viaAnthropic(log, server, anthropicKey);
    if (openaiKey) return await viaOpenAI(log, server, openaiKey);
  } catch (e) {
